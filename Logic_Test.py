...
communication_range = 4 # Number of vehicles ahead and behind within communication range
num_subchannels = 100
...
min_percent = 0.2
threshold = 3
vehicles_index_edge = [16, 17, 18, 19]
vehicle_index_central = [8,9,10,11]
prr_count_edge = 0
...
for i in range(4,19,4):
    left_vehicle = i - 1
    right_vehicle = i
    attacker_positions.append((left_vehicle, right_vehicle))
...
for attacker_id in range(num_vehicles, num_vehicles + num_attackers):
    ...
    start_idx = max(0, left_vehicle - communication_range + 1)
...
# # print(total_neighbors_edge)
# for vehicle, info in vehicles_info.items():
#     if vehicle in vehicles_index_edge:
#         print(f"Vehicle {vehicle} neighbors: {info['neighbors']}"  )
...
for subframe in tqdm(range(num_subframes), desc="Processing", ncols=100):
   ...
   # Step 3: Calculate Packet Delivery Ratio (PRR)over all vehicles every 2000 subframes
    ...
    if subframe % counting_interval == 0 and subframe != 0:
        prr_central = at.calculate_PRR(success_num_central_five,32)
